...
class Customer extends Model
{
    ...
    public function charge($amount, $makeOrder = true, $product = 'subscription', $coupon, $gift, $order_plan)
    {
        ...
        $price_discount = $this->getPlan()->getPriceDiscount();

        $upsell_discount =  $this->getPlan()->getUpsellDiscount();
        ...
        if($count_discount > 0){
            ...
            if($coupon->discount_type == "fixed"){
                $this->getPlan()->updatePriceFixed();
            }
        }


        if($coupon_free == 0 and $count_discount == 0 and $discount_type=='' and $upsell_discount > 0){
            $amount = $amount - ($amount * (50/100));
            $coupon = new Coupon();
            $coupon->code = "FRIENDDISCOUNT";
            $this->getPlan()->decreaseUpsellDiscount();
        }
    ...
    public function scopeRebillable($query)
    {
        return $query->join('plans', 'plans.id', '=', 'customers.plan_id')
            ...
            ->whereNull('plans.subscription_paused_at')
