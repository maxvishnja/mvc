...
class AccountController extends Controller
{
    ...
    function getHome()
    {
        ...
        $coupon = $plan->getCurrentCoupon();

        return view('account.my-takedaily', compact('orders', 'plan', 'nutritionist', 'coupon'));
    }
    ...
    function getSettingsDiscount()
    {
        $coupon = $this->customer->getPlan()->getUpsell();
        $count = $this->customer->getPlan()->getUpsellDiscount();

        return view('account.settings.discount', [
            'coupon' => $coupon,
            'count' => $count,
    ...
    function getSettingsSubscription()
    {
        ...
        $coupon = $this->customer->getPlan()->getCurrentCoupon();
...
//        });
    ...
    { // todo use a checkout model
        ...
        if($coupon->code == 'UNDSKYLD'){
            ...
            if($plan->last_coupon == $coupon->code or $coupon->code == $plan->upsell){
                return \Response::json(['message' => trans('checkout.messages.coupon-missing')], 400);
            } elseif($plan->coupon_free != ''){
        ...
        if($coupon->discount_type == 'percentage') {
            ...
            $plan->setDiscountCount($coupon->length);
            $plan->price_discount = $coupon->discount * $coupon->length * 100;
    ...
    public function changeCoupon(CouponRepository $couponRepository, Request $request){
        ...
        $coupon = $couponRepository->findByCouponForSecond($request->get('code'));

        if( $plan->clearAllDiscount()){
