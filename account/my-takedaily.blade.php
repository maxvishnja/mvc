	...
	<div class="row">
		<div class="col-md-7">
			...
			@if($customer->getPlan()->getCouponCount() > 0 and ($customer->getPlan()->getDiscountType() == 'month' or $customer->getPlan()->getDiscountType() == '' ) or ($customer->getPlan()->getDiscountCount() > 0 and  $customer->getPlan()->getPriceDiscount() == 0))
				...
				{!! trans('account.settings_subscription.total', [ 'amount' => trans('general.money-fixed-currency', ['amount' => 0, 'currency' => $plan->currency])]) !!}

			@elseif($customer->getPlan()->getCouponCount() > 0 and $customer->getPlan()->getDiscountType() == 'percent')

				{!! trans('account.settings_subscription.total', [ 'amount' => trans('general.money-fixed-currency', ['amount' => \App\Apricot\Libraries\MoneyLibrary::toMoneyFormat($plan->getTotal() - ($plan->getTotal() * ($customer->getPlan()->getCouponCount()/100)), true), 'currency' => $plan->currency])]) !!}

			@elseif($customer->getPlan()->getDiscountCount() > 0 and  $customer->getPlan()->getPriceDiscount() != 0)
				{!! trans('account.settings_subscription.total', [ 'amount' => trans('general.money-fixed-currency', ['amount' => \App\Apricot\Libraries\MoneyLibrary::toMoneyFormat($customer->getPlan()->getPriceDiscount(), true), 'currency' => $plan->currency])]) !!}

			@elseif($customer->getPlan()->getUpsellDiscount() > 0 and  $customer->getPlan()->getDiscountCount() == 0 and $customer->getPlan()->getDiscountType() == '' and $customer->getPlan()->getCouponCount() == 0)
			{!! trans('account.settings_subscription.total', [ 'amount' => trans('general.money-fixed-currency', ['amount' => \App\Apricot\Libraries\MoneyLibrary::toMoneyFormat($customer->getPlan()->getTotal() / 2, true), 'currency' => $plan->currency])]) !!}


			@else
				{!! trans('account.settings_subscription.total', [ 'amount' => trans('general.money-fixed-currency', ['amount' => \App\Apricot\Libraries\MoneyLibrary::toMoneyFormat($plan->getTotal(), true), 'currency' => $plan->currency])]) !!}
			@endif

				@if($plan->isActive() and $coupon)
					<small>({{ trans('account.home.current-coupon') }} {{ $coupon }})</small>
				@endif
			</h2>
			@if( $plan->isActive() )
			...
			<div class="m-t-20 m-b-20 m-center group-button">
				...
				@if( $plan->isActive() )
					<a href="{{ URL::action('AccountController@getSettingsSubscriptionPause') }}" class="button button--green">{{ trans('account.home.button-pause') }}</a>

				@else
						<a href="{{ URL::action('AccountController@getSettingsSubscriptionRestart') }}"
						   class="button button--green">{{ trans('account.settings_subscription.button-start-text') }}</a>
				@endif
	...
	<script>
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.message == 1){
                        swal({
                            title: "",
                            text: "<p style='font-size:18px'>{{ trans('account.coupon_change')}}</p><br/>" +
                            "<form method=\"post\" id='coupon-change'  action=\"{{ URL::action('AccountController@changeCoupon') }}\">" +
                            "<input type=\"hidden\" name=\"code\" value=\""+ response.coupon.code +"\" />" +
                            "<input type=\"hidden\" name=\"_token\" value=\"{{ csrf_token() }}\" />" +
                            "</form>",
                            type: "",
                            html: true,
                            confirmButtonText: "{{ trans('account.coupon_change_text') }}",
        ...
        $('#toggle-nutritionist-form').on('click', function (e) {
            ...
            swal({
                ...
                cancelButtonText: "{{ trans('popup.button-close') }}",
                confirmButtonColor: "#3AAC87",
                allowOutsideClick: true,
                showCancelButton: true,
                closeOnConfirm: false
